...
object SparkFunctions extends Serializable {
  ...
  val intervals: (scala.collection.mutable.LinkedHashMap[String, String], ZoneId, DateTimeFormatter) ⇒ java.util.LinkedHashMap[Period, String] =
    (stages, est, pattern) ⇒ {
      ...
      for ((k, v) ← stages) {
        if (start.isEmpty) {
          start = Option(ZonedDateTime.of(LocalDateTime.of(LocalDate.parse(v, pattern), time.LocalTime.of(23, 59, 59)), est))
          period = Option(k)
        } else {
          end = Option(ZonedDateTime.of(LocalDateTime.of(LocalDate.parse(v, pattern), time.LocalTime.of(23, 59, 59)), est))
