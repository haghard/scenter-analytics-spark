...
package object spark {
  ...
  sealed trait SparkQuery {
    def name: String
  }

  @simulacrum.typeclass trait StandingQuery[T] extends SparkQuery {
    def async(ctx: SparkContext, config: Config,
      teams: mutable.HashMap[String, String], period: String): Future[T]
  }

  @simulacrum.typeclass trait PtsLeadersQuery[T] extends SparkQuery {
    def async(ctx: SparkContext, config: Config, period: String, depth: Int): Future[T]
  }

  @simulacrum.typeclass trait RebLeadersQuery[T] extends SparkQuery {
    def async(ctx: SparkContext, config: Config, period: String, depth: Int): Future[T]
  }

  @simulacrum.typeclass trait PlayerStatsQuery[T] extends SparkQuery {
    def async(ctx: SparkContext, config: Config, name: String, period: String, team: String): Future[T]
  }

  @simulacrum.typeclass trait TeamsResultsQuery[T] extends SparkQuery {
  ...
  @simulacrum.typeclass trait DailyResultsQuery[T] extends SparkQuery {
  ...
  object DailyResultsQuery {
    ...
    implicit def teamStats(implicit ex: ExecutionContext) = new DailyResultsQuery[DailyResultsView] {
  ...
  object TeamsResultsQuery {
    implicit def teamStats(implicit ex: ExecutionContext) = new TeamsResultsQuery[ResultsView] {
      ...
      override def async(ctx: SparkContext, config: Config, period: String, teams: scala.collection.Seq[String], arenas: Seq[(String, String)],
        ...
        Future {
          ...
          val resultsDF = ctx.cassandraResultByPeriodRdd(config, allTeams.keySet, period).cache()
        ...
        }.flatMap(_.collectAsync.map(res ⇒ ResultsView(res.size, res.toList)))
    }
  }
  ...
  object PlayerStatsQuery {
    ...
    implicit def playersStats(implicit ex: ExecutionContext) = new PlayerStatsQuery[PlayerStatsView] {
  ...
  object PtsLeadersQuery {
    implicit def ptsLeaders(implicit ex: ExecutionContext) = new PtsLeadersQuery[PtsLeadersView] {
      ...
      override def async(ctx: SparkContext, config: Config, period: String, depth: Int): Future[PtsLeadersView] =
        Future {
          val rdd: RDD[((String, String), Float)] = ctx.cassandraPtsLeadersRdd(config, period)
          val array = rdd
            .combineByKey(
              (pts: Float) ⇒ (pts, 1l),
          ...
          PtsLeadersView(
            array.length,
            array.map(kv ⇒ PtsLeader(team = kv._1._2, player = kv._1._1, pts = kv._2._1.scale(2), games = kv._2._2)).toList
          )
        }
    }
  }
