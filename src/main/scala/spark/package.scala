    ...
    implicit def teamStats(implicit ex: ExecutionContext) = new TeamsResultsQuery[TeamStatsView] {
      ...
      override def async(ctx: SparkContext, config: Config, period: String, teams: scala.collection.Seq[String],
        ...
        ((keyTeamsDF join (resultsDF, resultsDF("home-team") === keyTeamsDF("team") || resultsDF("away-team") === keyTeamsDF("team"))) join (arenasDF, "home-team"))
          .sort(resultsDF("date")).map { row: Row ⇒
    ...
    implicit def instance(implicit ex: ExecutionContext) = new RebLeadersQuery[RebLeadersView] {
      ...
      override def async(ctx: SparkContext, config: Config, period: String, depth: Int): Future[RebLeadersView] = {
        ...
        val array = rdd.combineByKey(
          ...
          (res: ((Float, Float, Float), Long), tuple: (Float, Float, Float)) ⇒
            ((res._1._1 + tuple._1, res._1._2 + tuple._2, res._1._3 + tuple._3), res._2 + 1),
          (left: ((Float, Float, Float), Long), right: ((Float, Float, Float), Long)) ⇒
            ((left._1._1 + right._1._1, left._1._2 + right._1._2, left._1._3 + right._1._3), left._2 + right._2)
        ).mapValues({
            case ((offReb, defReb, totalReb), count) ⇒ if (count > 0)
              (((offReb / count).toDouble, (defReb / count).toDouble, (totalReb / count).toDouble), count)
    ...
    implicit def ptsLeaders(implicit ex: ExecutionContext) = new PtsLeadersQuery[PtsLeadersView] {
      ...
      override def async(ctx: SparkContext, config: Config, period: String, depth: Int): Future[PtsLeadersView] = {
        ...
        val array = rdd.combineByKey(
          ...
          (res1: (Float, Long), res2: (Float, Long)) ⇒ (res1._1 + res2._1, res1._2 + res2._2)
        ).mapValues({ case (sum, count) ⇒ if (count > 0) ((sum / count).toDouble, count) else (0.0, count) })
  ...
  object StandingQuery {
    ...
    implicit def playoff(implicit ex: ExecutionContext) = new StandingQuery[PlayoffStandingView] {
      ...
      override def async(ctx: SparkContext, config: Config, teams: mutable.HashMap[String, String], period: String): Future[PlayoffStandingView] = {
        ...
        */
        ...
        rdd.keyBy(v ⇒ Set(v.homeTeam, v.awayTeam))
          .aggregateByKey(TreeMap[Date, String]())((key, r) ⇒
            if (r.homeScore > r.awayScore) key + (r.dt -> s"[${r.homeScore}:${r.awayScore} ${r.homeTeam} won]")
            else key + (r.dt -> s"[${r.homeScore}:${r.awayScore} ${r.awayTeam} won]"),
            (m0, m1) ⇒ m0 ++ m1)
          .map { kv ⇒ (kv._2.lastKey, kv._1.head + "-" + kv._1.last + "  " + kv._2.values.mkString(", ")) }
          .sortByKey()
          .map(_._2)
    ...
    implicit def season(implicit ex: ExecutionContext) = new StandingQuery[SeasonStandingView] {
      ...
      override def async(ctx: SparkContext, config: Config, teams: mutable.HashMap[String, String], period: String): Future[SeasonStandingView] = {
        ...
        rdd.flatMap { r ⇒
          if (r.homeScore > r.awayScore) List((r.homeTeam, "hw"), (r.awayTeam, "al")) else List((r.homeTeam, "hl"), (r.awayTeam, "aw"))
        }.aggregateByKey(Standing())(
          (acc: Standing, state: String) ⇒ state match {
