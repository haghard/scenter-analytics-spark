...
trait SecurityRouter extends DefaultRestMicroservice with Directives {
  ...
  lazy val (google, twitter, github) = {
    (http.oauth
       .Oauth[com.github.scribejava.apis.GoogleApi20]
       .withKeySecret(googleApiKey, googleApiSecret),
     http.oauth
       .Oauth[com.github.scribejava.apis.TwitterApi]
       .withKeySecret(twitterApiKey, twitterApiSecret),
     http.oauth
       .Oauth[com.github.scribejava.apis.GitHubApi]
       .withKeySecret(githubApiKey, githubApiSecret))
  }

  implicit def serializer: SessionSerializer[ServerSession, String] =
    new SingleValueSessionSerializer({ session: ServerSession ⇒
  ...
  val sessionConfig =
    SessionConfig.default(system.settings.config.getString("http.secret"))
  implicit val sessionManager =
    new SessionManager[ServerSession](sessionConfig)

  implicit val refreshTokenStorage =
    new InMemoryRefreshTokenStorage[ServerSession] {
  ...
  abstract override def configureApi() =
    super.configureApi() ~
      Api(route = Option { ec: ExecutionContext ⇒
          ...
          urls =
            s"[$httpPrefixAddress/$pathPrefix/login, $httpPrefixAddress/$pathPrefix/login-twitter, $httpPrefixAddress/$pathPrefix/login-github, $httpPrefixAddress/$pathPrefix/login-google, $httpPrefixAddress/$pathPrefix/logout]")

  private def githubR(implicit ec: ExecutionContext): Route =
    path("login-github") {
      val service = github.oAuthService
        .callback(s"http://$domain:$httpPort/$pathPrefix/github-sign-in")
        .build(GitHubApi.instance)
      // Obtain the Authorization URL
      val url = service.getAuthorizationUrl()
      redirect(akka.http.scaladsl.model.Uri(url),
      ...
      get {
        extractHost { host ⇒
          ...
          val service = github.oAuthService
            .callback(s"http://$host:9000/github-callback")
            .build(GitHubApi.instance)
        ...
        parameterMap { params ⇒
          complete {
            Future {
              val service = github.oAuthService
                .callback(
                    s"http://$domain:$httpPort/$pathPrefix/github-sign-in")
                .build(GitHubApi.instance)
              val (k, v) = params.head
              ...
              val request =
                new OAuthRequest(Verb.GET, github.protectedUrl, service)
  ...
  private def googleR(implicit ec: ExecutionContext): Route =
    path("login-google") {
      val service = google.oAuthService
        .callback(s"http://$domain:$httpPort/$pathPrefix/google-sign-in")
        ...
        parameterMap { params ⇒
          complete {
            Future {
              val service = google.oAuthService
                .callback(
                    s"http://$domain:$httpPort/$pathPrefix/google-sign-in")
                .build(GoogleApi.instance)
              val (k, v) = params.head
              val verifier = new com.github.scribejava.core.model.Verifier(v)
              ...
              val request =
                new OAuthRequest(Verb.GET, google.protectedUrl, service)
              service.signRequest(accessToken, request)
              ...
              if (response.getCode == 200) {
                ...
                val user =
                  json.fields("displayName").toString().replace("\"", "")
    ...
    path("login-twitter") {
      get {
        val service = twitter.oAuthService
          .callback(s"http://$domain:$httpPort/$pathPrefix/twitter-sign-in")
          ...
          (oauthToken, oauthVerifier) ⇒
            ...
            complete {
              Future {
                ...
                val requestToken =
                  new com.github.scribejava.core.model.Token(oauthToken,
                                                             oauthVerifier)
                val verifier =
                  new com.github.scribejava.core.model.Verifier(oauthVerifier)
                val accessToken =
                  service.getAccessToken(requestToken, verifier)
                val oAuthRequest =
                  new com.github.scribejava.core.model.OAuthRequest(
                      Verb.GET,
                      twitter.protectedUrl,
                      service)
  ...
  def securityRoute(implicit ec: ExecutionContext): Route = {
    pathPrefix(pathPrefix) {
      path("login") {
        get {
          parameters(('user.as[String]), ('password.as[String])) {
            (user, password) ⇒
              withUri { url ⇒
                setSession(
                    oneOff,
                    usingHeaders,
                    ServerSession(user, BCrypt.hashpw(password, salt))) {
                  setNewCsrfToken(checkHeader) { ctx ⇒
                    ctx.complete(s"$user was logged in")
                  }
                }
              }
          }
