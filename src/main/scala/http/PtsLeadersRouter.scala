...
@io.swagger.annotations.Api(value = "pts", produces = "application/json")
...
class PtsLeadersRouter(override val host: String, override val httpPort: Int,
  ...
  @Path("/{stage}")
  @ApiOperation(value = "Search stats by stage", notes = "", httpMethod = "GET")
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "Authorization", value = "Authorization token", required = true, dataType = "string", paramType = "header"),
    new ApiImplicitParam(name = "stage", value = "Stage of results", required = true, dataType = "string", paramType = "path"),
  ...
  def searchReb(url: String, stage: String, depth: Int): Future[HttpResponse] = {
  ...
  def searchPts(url: String, stage: String, depth: Int): Future[HttpResponse] = {
    validatePeriod(stage).fold({ error => Future.successful(notFound(s"Invalid parameters: $error")) }, { _ =>
