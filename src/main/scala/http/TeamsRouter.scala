...
trait TeamsRouter extends SecurityRouter with TypedAsk with TeamsHttpProtocols { mixin: MicroKernel ⇒
  ...
  abstract override def configureApi() =
    super.configureApi() ~
      Api(route = Option { ec: ExecutionContext ⇒ resultsRoute(ec) },
        ...
        urls = s"[$httpPrefixAddress/$pathPrefix/$teamsServicePath/{stage}]")
      ...
      (get & path(teamsServicePath / Segment)) { period ⇒
        parameters(('teams.as[String])) { teams ⇒
          requiredHttpSession(ec) { session ⇒
            withUri { url ⇒
              system.log.info(s"[user:${session.user}] access [$httpPrefixAddress/$pathPrefix/$teamsServicePath/{stage}]")
  ...
  private def search(url: String, stage: String, searchTeams: String)(implicit ex: ExecutionContext): Future[HttpResponse] = {
